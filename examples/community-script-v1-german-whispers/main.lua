-- Community Variant Script API V1 German Whispers reference package.
-- Community Variant Script API V1 德国耳语数独参考包。
--
-- Path geometry is puzzle-owned. This package fixes the semantic threshold:
-- orthogonally adjacent cells on a path must differ by at least five. The Host
-- owns board state, candidates, completion, persistence and rendering.
local plugin = community_variant.script()
local german_whispers = {}
...
local function normalize_paths(config)
  if type(config) ~= "table" or type(config.paths) ~= "table" then
    error("german_whispers requires data.paths")
  end
  local paths = {}
  local pairs_by_cell = {}
  local used_edges = {}
  for path_index, raw_path in ipairs(config.paths) do
    if type(raw_path) ~= "table" or #raw_path < 2 then
      error("german_whispers path " .. path_index .. " requires at least two cells")
    end
    local path = {}
    local used_cells = {}
    for cell_index, raw_cell in ipairs(raw_path) do
      local cell = cell_api.expect(raw_cell, "german_whispers path cell")
      if used_cells[cell] then
        error("german_whispers path must not repeat a cell")
      end
      ...
      if cell_index > 1 then
        ...
        if not adjacency.orthogonal(previous, cell) then
          error("german_whispers path cells must be orthogonal neighbors")
        end
        local edge = path_api.edge_key(previous, cell)
        if used_edges[edge] then
          error("german_whispers paths must not repeat an edge")
        end
  ...
  if #paths == 0 then
    error("german_whispers requires at least one path")
  end
  ...
  for _, path in ipairs(model.paths) do
    for index = 1, #path - 1 do
      ...
      if not board.is_empty(first) and not board.is_empty(second)
          and math.abs(first - second) < 5 then
        violations[#violations + 1] = {
          code = "german_whispers_minimum_difference",
...
local function candidate_allowed(model, ctx, cell, digit)
  ...
  for _, pair in ipairs(pairs) do
    ...
    if not board.is_empty(other_digit) and math.abs(digit - other_digit) < 5 then
      return false
    end
  end
  return true
...
function german_whispers.create(config, scope)
    ...
    get_candidate_eliminations = function(ctx, cell)
      ...
      for digit = 1, 9 do
        if not candidate_allowed(model, ctx, cell, digit) then
          ...
          reasons[tostring(digit)] = "german_whispers_minimum_difference"
        end
      end
...
plugin:register_rule("german_whispers", german_whispers)
