-- Community Variant Script API V1 Non-Consecutive reference implementation.
-- Community Variant Script API V1 非连续数独参考实现。
--
-- The rule handler owns only the Non-Consecutive semantics. The Host owns the
-- board, the move transaction, classic Sudoku validation, candidates, notes,
-- conflicts, persistence and the gameplay lifecycle.
-- handler 只拥有非连续规则语义。棋盘、落子事务、经典数独校验、候选、笔记、冲突、
-- 持久化和游戏生命周期仍由 Host 负责。
local plugin = community_variant.script()
local non_consecutive = {}
local board = community_variant.board

local function orthogonal_offsets()
  -- Two directed offsets expand to every undirected orthogonal-adjacent pair
  -- on a 9x9 board without duplicates.
  -- 两个方向展开后覆盖 9x9 棋盘上的所有无向正交相邻 pair，且不会重复。
  return {
...
local function make_violation(first, second)
  return {
    code = "non_consecutive_adjacent",
    cells = { first, second }
  }
end

local function are_consecutive(first_digit, second_digit)
...
local function find_violations(model, ctx, override_cell, override_digit)
  ...
  for _, pair in ipairs(model.pairs) do
    ...
    -- Partial board semantics: an adjacent pair is not false until both cells
    -- are filled. Empty cells remain undetermined.
    -- 部分填盘语义：只有相邻两个格子都填入数字后，才判断连续数字违规。
    if not board.is_empty(first_digit)
        and not board.is_empty(second_digit)
        and are_consecutive(first_digit, second_digit) then
...
function non_consecutive.create(config, scope)
  -- Orthogonal adjacency is package-owned. Empty data prevents individual
  -- puzzles from silently changing the published meaning of Non-Consecutive.
  -- 正交相邻关系属于包级语义。空 data 防止单个题目偷偷改变非连续定义。
  if type(config) ~= "table" or next(config) ~= nil then
    error("non_consecutive requires empty rule data")
  end

  local pairs, by_cell = expand_pairs(orthogonal_offsets())
  local model = {
    pairs = pairs,
    by_cell = by_cell
  }
  ...
  return {
    ...
    -- Report current-board Non-Consecutive violations for the Host conflict layer.
    -- 把当前盘面的非连续违规交给 Host 冲突层。
    ...
    get_candidate_eliminations = function(ctx, cell)
      ...
      if neighbors ~= nil then
        for _, neighbor in ipairs(neighbors) do
          ...
          if not board.is_empty(digit) then
            mark_candidate_removal(remove_set, digit - 1)
            mark_candidate_removal(remove_set, digit + 1)
          end
        end
      end
      ...
      for digit = 1, 9 do
        if remove_set[digit] == true then
          ...
          reasons[tostring(digit)] = "non_consecutive_adjacent"
        end
      end
    ...
    -- Completion remains Host-owned; this reports only Non-Consecutive validity.
    -- 完成流程仍归 Host；这里仅报告非连续规则是否有效。
...
plugin:register_rule("non_consecutive", non_consecutive)
